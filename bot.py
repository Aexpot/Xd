...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.debug(f"✅ Сообщение с выбором типа отправлено, message_id: {message.message_id}")
        
        return CHOOSE_TYPE
    except Exception as e:
        logger.error(f"❌ Ошибка в report_command: {e}", exc_info=True)
...
async def confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"❌ Ошибка в процессе отправки: {str(e)}", exc_info=True)
...
• Не злоупотребляйте функцией репортов
        ...
        logger.error(f"❌ Ошибка при обработке репортов: {str(e)}", exc_info=True)
        await query.edit_message_text(f"❌ **Ошибка:** {str(e)}")
        return False

async def code_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик ввода кода подтверждения"""
    # Проверяем, ожидаем ли мы код от этого пользователя
    if not context.user_data.get('waiting_for_code'):
        # Если не ожидаем, пропускаем обработку
        return
...
def main():
    ...
    # Хендлер для кодов подтверждения (добавляем ПОСЛЕ ConversationHandler)
    ...
    print("👁️  Следите за логами в консоли и в файле bot.log...")
    print("=" * 60)
